...
%
%DESCRIPTION:
%Step 1 of the AUTOMATE algorithm, generates possible paths between the
%departure planet and the arrival planet with the given infinity velocities
%
%INPUTS
%-dep_planet:
...
%--------------------------------------------------------------------------
    ...
    if ~ismember(dep{1}(3),v_inf_levels)
        disp('Error, starting Vinf not in V_inf_levels');
        paths_final=[];
        return
    end
        ...
        elseif depth>=max_depth
            ...
            for i=size(new_nodes,1):-1:1
                ...
                if tmp(1)~=dep_planet || tmp(2)>ini_vinf
                    ...
                    if size(temp_seq_id,1)>=2 && ~ismember(tmp,temp_seq_id(end-1:end,:),'rows')
    ...
    paths_final=paths(~cellfun('isempty',paths));
