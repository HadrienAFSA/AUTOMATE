function [ToF_Paths] = generate_PathsToF(fixed_Paths, Grid, phase_database)
%generate_PathsToF.m - Generate paths Time of Flight
%
%DESCRIPTION:
%Generates all the possible ToF of each leg and for all transfer types (I/O).
%
%INPUTS
%-fixed_Paths:
...
%   applicable), the ToF matrix for all transfers types and all legs,
%   in seconds, and the potential launch window
%
%AUTHOR
%Hadrien AFSA
%
    ...
    for i=1:size(fixed_Paths,1)
        ...
        for s=2:length(seq)
            ...
            if prev(1)==curr(1)
                ...
                % Computes the SC orbit parameters
                ...
                elseif curr(1)<prev(1)            
                    ...
                    error("Error in generate_PathsToF");
                end

                % Saves the different ToF (in previous order)
                legs_ToF(s-1,:)= [ToF_OO ToF_OI ToF_IO ToF_II];
            end

            % Finds the launch date, only for the first leg
            if s==2
                launch_window=find_launchWindow(prev(1), curr(1), theta_1, theta_2, legs_ToF(1,:), phase_database);
            end
        end
        
        % Saves the resonances
        ToF_Paths{i,2}=fixed_Paths{i,2};
        % Saves all the ToFs 
        ToF_Paths{i,3}=legs_ToF;
        % Saves the possible launch dates
        ToF_Paths{i,4}=launch_window;
        % Saves the DSMs
        ToF_Paths{i,5}=fixed_Paths{i,4};
